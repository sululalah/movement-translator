...
(function(){
  ...
  function abFade(){const fv=parseInt(abFadeSl.value);if(fv<=0)return;mCtx.globalAlpha=fv/500;mCtx.globalCompositeOperation='source-over';mCtx.fillStyle='#fff8f0';mCtx.fillRect(0,0,abCanvas.width,abCanvas.height);mCtx.globalAlpha=1;mCtx.globalCompositeOperation='source-over';}
  ...
  function abResize(){const p=abCanvas.parentElement,w=p.clientWidth,h=p.clientHeight;[abCanvas,abGhost,abM2Ov].forEach(c=>{c.width=w;c.height=h;});mCtx.fillStyle='#fff8f0';mCtx.fillRect(0,0,w,h);const fp=abPoseOv.parentElement;abPoseOv.width=fp.clientWidth;abPoseOv.height=fp.clientHeight;}
  ...
  m1Rec.addEventListener('click',()=>{if(!abIsRec){abStartRec();m1Rec.classList.add('recording');m1Rec.innerHTML='<span class="record-dot"></span>Stop & Save';abSetSt('Recording…',true);}else{abStopRec();m1Rec.classList.remove('recording');m1Rec.innerHTML='<span class="record-dot"></span>Record MP4';abSetSt('Saved',true);}});
  ...
  document.getElementById('ab-m2-file-input').addEventListener('change',e=>{const f=e.target.files[0];if(!f)return;abGhostVid.src=URL.createObjectURL(f);abGhostVid.onloadedmetadata=()=>{abGhostLoaded=true;abGhost.style.display='block';abSetSt('Ghost loaded',true);};});
  const m2PlayBtn=document.getElementById('ab-m2-play-ghost');
  m2PlayBtn.addEventListener('click',()=>{if(!abGhostLoaded)return;if(abGhostVid.paused){abGhostVid.play();m2PlayBtn.textContent='⏸ Pause Ghost';(function ghostTick(){if(!abGhostVid.paused&&!abGhostVid.ended){gCtx.clearRect(0,0,abGhost.width,abGhost.height);gCtx.drawImage(abGhostVid,0,0,abGhost.width,abGhost.height);requestAnimationFrame(ghostTick);}})();}else{abGhostVid.pause();m2PlayBtn.textContent='▶ Play Ghost';}});
  let abM2Cam=null,abM2BP=null,abM2Active=false,abM2Raf=null,abM2Det=false;
  document.getElementById('ab-m2-start-cam-btn').addEventListener('click',async()=>{abSetSt('Starting artboard camera…');try{abM2Cam=await navigator.mediaDevices.getUserMedia({video:{width:640,height:480,facingMode:'user'},audio:false});abM2VidEl.srcObject=abM2Cam;await abM2VidEl.play();abM2Active=true;abM2Ov.style.display='block';document.getElementById('ab-m2-start-cam-btn').style.display='none';document.getElementById('ab-m2-stop-cam-btn').style.display='inline-flex';document.getElementById('ab-m2-cam-status').textContent='● Camera live on artboard';abM2BP=await new Promise(res=>{const bp=ml5.bodyPose('MoveNet',{flipped:false},()=>res(bp));});abSetSt('Artboard camera tracking',true);(function m2Loop(){abM2Raf=requestAnimationFrame(m2Loop);if(!abM2VidEl||abM2VidEl.readyState<2||abM2Det)return;if(!abPrepFrame(abM2VidEl,abS2C,abS2Ctx))return;const sw=abS2C.width,sh=abS2C.height;abM2Det=true;abM2BP.detect(abS2C,(poses,err)=>{abM2Det=false;if(err)return;abSkeleton(m2Ctx,poses,abM2Ov.width,abM2Ov.height,sw,sh,true);abPaint(poses,sw,sh,true);});})();}catch(err){abSetSt('Artboard camera error: '+err.message);}});
  function abStopM2Cam(){abM2Active=false;abM2Det=false;if(abM2Raf){cancelAnimationFrame(abM2Raf);abM2Raf=null;}abM2BP=null;if(abM2Cam){abM2Cam.getTracks().forEach(t=>t.stop());abM2Cam=null;}abM2VidEl.srcObject=null;m2Ctx.clearRect(0,0,abM2Ov.width,abM2Ov.height);abM2Ov.style.display='none';document.getElementById('ab-m2-start-cam-btn').style.display='inline-flex';document.getElementById('ab-m2-stop-cam-btn').style.display='none';document.getElementById('ab-m2-cam-status').textContent='';}
  ...
  m2Rec.addEventListener('click',()=>{if(!abIsRec){abStartRec();m2Rec.classList.add('recording');m2Rec.innerHTML='<span class="record-dot"></span>Stop & Save';abSetSt('Recording follow-along…',true);}else{abStopRec();m2Rec.classList.remove('recording');m2Rec.innerHTML='<span class="record-dot"></span>Record MP4';abSetSt('Saved',true);}});
  document.getElementById('ab-m2-clear-btn').addEventListener('click',()=>{mCtx.fillStyle='#fff8f0';mCtx.fillRect(0,0,abCanvas.width,abCanvas.height);abResetTrails();gCtx.clearRect(0,0,abGhost.width,abGhost.height);});
  abResize();
